        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <br>
                <div class="max-w-5xl mx-auto sm:px-6 lg:px-4">
                    <form id="addImages" name="addImages" action="{{ route('hotel_images.store') }}" method="post" enctype="multipart/form-data">
                    <fieldset>
                        ...
                        <div class="form-row">
                            <div class="col col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control outline-none" id="txtSeqNo" name="txtSeqNo" value="{{ old('txtSeqNo', $seqNo) }}" min="1" required>
                                    									
									@else
										
                                    <input type="hidden" class="form-control" id="txtId" placeholder="Id" name="txtId" value="{{ $data['id']  }}" autofocus required />
                                    <input type="text" class="form-control" id="txtName" placeholder="Name" name="txtName" value="{{ $data['name']  }}" min="1" required />
                                    </div>
                                <div class="form-group">
                                    <label for="txtName">{{__('hotel.seq-no')}}</label><label style="color:red">* </label>
                                    <input type="number" class="form-control" id="txtSeqNo" name="txtSeqNo" value="{{ $data['seq_no'] ? $data['seq_no'] : $seqNo }}" min="1" required />
                                    @endif
                            ...
                            <div class="col col-md-6">
                                <div class="form-group">
                                        ...
                                        <label for="Maps">{{__('hotel.image')}}</label><label style="color:red">* </label>
                                        @if (empty($data))
                                        <input class="form-control form-control-sm" name="images" id="images" type="file" required accept=".jpg, .jpeg, .png" onchange="validate_input(event, this, ['.jpg','.jpeg','.png'])" value="{{ old('images') }}" />
									...
									<input type='hidden' name="oldImages" id="Oldimages" value="{{$data['file_name']}}"/>
                                        <input class="form-control form-control-sm" type='file' name="images" id="images" value="{{url($data['file_name'])}}" accept=".jpg, .jpeg, .png" onchange="validate_input(event, this, ['.jpg','.jpeg','.png'])" />
                                        <label style="font-size: 12px;">{{__('hotel.image-desc')}}</label>
                                        ...
                                        <img id="icon" src="{{url($data['file_name'])}}" alt="your image" style="width:100%;height:380px;" />
                                        @endif
		...
		var sFileName = $(oInput)[0].files[0].name;
		 if (sFileName.length > 0) {
			...
			if (!blnValid) {
				...
				message = message.replace('[extensions]',_validFileExtensions);
